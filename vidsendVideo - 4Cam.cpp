...
void CTV::preConstruct(CWnd *pWnd) {
	...
	*imposeText=0;
	...
	m_DShow=NULL;
	m_RTSPsocket=NULL;
	ThreadHandleRTSPV=ThreadHandleRTSPA=NULL;		// handle ai task RTSP, 2020
	endThreadV=endThreadA=FALSE;
	ThreadRTSPVID=ThreadRTSPAID=0;
...
no_hwnd:
		...
		}			// se qualeInput va su USB

		else {		// RTSP
			int width = biRawBitmap.biWidth; //928 o 704 o 352
			int height = biRawBitmap.biHeight;	//288; //576
			...
			m_hWnd = CreateWindowEx(0, wc.lpszClassName, "RTSP view", 
										WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 
										0, 0, 
										r.right-r.left, r.bottom-r.top,
										m_Parent->m_hWnd, 0, AfxGetInstanceHandle() /*theApp.m_hInstance*/, 0);
	...
	//		i=SetTimer(hWnd,1,100,NULL);
			...
			biRawBitmap.biWidth=width;
			biRawBitmap.biHeight=height;
			biRawBitmap.biSizeImage=(biRawBitmap.biWidth*biRawBitmap.biHeight*biRawBitmap.biBitCount)/8;
			biRawBitmap.biCompression=0;
			biRawBitmap.biBitCount=24;
			biBaseRawBitmap=biRawBitmap;
			biBaseRawBitmap.biCompression=0;
...
/*		CString info;
		...
		maxFrameSize=max(maxFrameSize /*2018, o usciva 3:4 del max... PERCHE' MANCAVA INDEO50! cmq lo lascio */,
			ICCompressGetSize(m_hICCo,&biBaseRawBitmap,&biCompDef.bmiHeader));
		i=ICCompressBegin(m_hICCo,&biBaseRawBitmap,&biCompDef.bmiHeader);
...
//		KillTimer(hWnd,1);
  ...
  theApp.prStore->WriteProfileVariabileInt(IDS_WHICH_CAMERA,theCapture);
	...
	if(m_RTSPsocket) {
		...
		while(ti>timeGetTime() && endThreadV)
			Sleep(100);
		delete m_RTSPsocket;
		}
	m_RTSPsocket=NULL;
...
RECT *CTV::Resize(RECT *r, BOOL force) {
	...
	if(m_hWnd) {
		if(inCapture==1) {		// a volte arriva Resize MENTRE e' in Capture (da createView o altro...) obbligatorio sincronizzare le cose!
			oldC=inCapture;
			if(qualeInput<2) {		// con RTSP dà fastidio e non serve!
				Capture(0);
				}
			}
		if(force) {
	...
	if(m_hWnd) {
		...
		if(m && inCapture!=1) {
			...
			if(qualeInput<2) {
				...
				if(!m_RTSPsocket) {		// safety. diciamo
					m_RTSPsocket=new CRTSPClientSocket("rtsp://192.168.1.10:554/user=dario_password=dario_channel=1_stream=1.sdp"); 
				//	m_RTSPsocket.DoOPTIONS();			// VLC usa options e non describe
					m_RTSPsocket->DoDESCRIBE();
					m_RTSPsocket->ParseSDP();
					i=m_RTSPsocket->DoSETUP("video", false /*true per tcp*/);
					m_RTSPsocket->DoPLAY();
//					hCloseEventRTSPV = CreateEvent(NULL, TRUE, FALSE, NULL);
					m_RTSPsocket2=new CRTSPClientSocket("rtsp://192.168.1.10:554/user=dario_password=dario_channel=2_stream=1.sdp"); 
					m_RTSPsocket2->DoDESCRIBE();
					m_RTSPsocket2->ParseSDP();
					i=m_RTSPsocket2->DoSETUP("video", false /*true per tcp*/);
					m_RTSPsocket2->DoPLAY();
					m_RTSPsocket3=new CRTSPClientSocket("rtsp://192.168.1.10:554/user=dario_password=dario_channel=3_stream=1.sdp"); 
					m_RTSPsocket3->DoDESCRIBE();
					m_RTSPsocket3->ParseSDP();
					i=m_RTSPsocket3->DoSETUP("video", false /*true per tcp*/);
					m_RTSPsocket3->DoPLAY();
					m_RTSPsocket4=new CRTSPClientSocket("rtsp://192.168.1.10:554/user=dario_password=dario_channel=4_stream=1.sdp"); 
					m_RTSPsocket4->DoDESCRIBE();
					m_RTSPsocket4->ParseSDP();
					i=m_RTSPsocket4->DoSETUP("video", false /*true per tcp*/);
					m_RTSPsocket4->DoPLAY();
					ThreadHandleRTSPV = CreateThread(NULL, 0,
						(LPTHREAD_START_ROUTINE)RTSPvideo_Thread,(void*)theApp.theServer/*m_RTSPsocket*/,0,&ThreadRTSPVID);
//					SetThreadPriority(ThreadHandleRTSPV ,THREAD_PRIORITY_HIGHEST);
					}
				}	
			return 1;
			}
...
//						SignalObjectAndWait(hCloseEventRTSPV,ThreadHandleRTSPV,5000,FALSE); non c'è pd
					endThreadV=TRUE;
					DWORD ti=timeGetTime()+500;
					while(ti>timeGetTime() && endThreadV)
						Sleep(100);
//						if((WaitForSingleObject(ThreadHandleRTSPV, 500)) == WAIT_TIMEOUT) 		// a volte non va...
					if(endThreadV)		// niente, al primo giro (capture implicito in open) non va... ma ok
						TerminateThread(ThreadHandleRTSPV,(DWORD)-1);
					ThreadHandleRTSPV=NULL;
					endThreadV=FALSE;
/*					if(hCloseEventRTSPA) {
						SetEvent(hCloseEventRTSPA);
	//				TerminateThread(ThreadHandleRTSPA,0);
					// (OCCHIO a memory leaks, usare Signal! e poi attenzione alla memoria passata al compressore ecc quando viene disallocato RTSP
						CloseHandle(hCloseEventRTSPA);
						hCloseEventRTSPA=NULL;
						}*/
					m_RTSPsocket->DoTEARDOWN();
					m_RTSPsocket->Close();
					delete m_RTSPsocket;
					m_RTSPsocket=NULL;
					m_RTSPsocket2->DoTEARDOWN();
					m_RTSPsocket2->Close();
					delete m_RTSPsocket2;
					m_RTSPsocket2=NULL;
					m_RTSPsocket3->DoTEARDOWN();
					m_RTSPsocket3->Close();
					delete m_RTSPsocket3;
					m_RTSPsocket3=NULL;
					m_RTSPsocket4->DoTEARDOWN();
					m_RTSPsocket4->Close();
					delete m_RTSPsocket4;
					m_RTSPsocket4=NULL;
					}
				}
			return 1;
...
BOOL CTV::GetDriverDescription(WORD wDriverIndex,  
  ...
  LPSTR lpszVer, int cbVer) {
	...
	if(qualeInput<2) {
		...
		CString S=theApp.RTSPServer+" "+theApp.RTSPUsername+" "+theApp.RTSPPassword+"/"+	theApp.RTSPStream;
		_tcsncpy(lpszName,(LPCTSTR)S,cbName);			// mettere user pasw ecc
		_tcsncpy(lpszVer,"RTSP client",cbVer);
		return TRUE;
		}
	}
...
int CTV::superImposeText(const CBitmap *b,const char *s,COLORREF dColor,bool flag) {
	int xSize,ySize;
//	int x,y;
	int i,n;
//	BYTE *p,*p1;
...
//		yStep=-bi->biWidth*3;
	switch(k & 3) {
		case 0:			// sinistra in alto
			xStart=8;
			yStart=bi->biHeight-bi->biHeight/20;
			break;
		case 1:			// destra in alto
			xStart=bi->biWidth- (k & 4 ? xSize*12 : xSize*12);		// finire... con la dimensione grande non va bene!
			yStart=bi->biHeight-bi->biHeight/20;
			break;
		case 2:			// destra in basso
			xStart=bi->biWidth- (k & 4 ? xSize*12 : xSize*12);
			yStart=bi->biHeight/10;
			break;
		case 3:			// sinistra in basso
			xStart=8;
			yStart=bi->biHeight/10;
...
//ASSERT(0);
	...
	b.CreateBitmap(xSize,ySize+1,1,1 /*24  la bitmap la vuole per forza in b/n... */,NULL);
	...
	if(n2) {
		switch(bi->biBitCount) {
			case 24:
				p2+=(xSize/8)*3;			// salto qualche righina  SAREBBE ANCHE CARINO aggiungere un po' di bordo sopra e sotto
//				for(y=(ySize-20+yStart); y>1-18+yStart; y--) {
				for(y=(ySize-20+yStart); y>=1-18+yStart; y--) {
//				for(y=(ySize-20+yStart); y>yStart; y--) {		// rimpicciolito per rtsp 352*288 2026
...
//							dValue += dRGB;
					...
					if(cnt0) {		// p.es. se ccordinate qualitybox vuote o fuori schermo
...
//	CTV *m_TV=(CTV *)capGetUserData(hWnd) /* non e' lpVHdr->dwUser !! */;  FA CAGARE!
	...
	switch(uMsg) {
		case WM_CREATE:
			ZeroMemory(&m_TV->bmih_draw, sizeof(BITMAPINFOHEADER));
			m_TV->bmih_draw.biSize = sizeof(BITMAPINFOHEADER);
			m_TV->bmih_draw.biPlanes = 1;
			if(m_TV->m_DShow) {
				m_TV->bmih_draw.biBitCount = m_TV->m_DShow->GetBitsPerPixel();
				m_TV->bmih_draw.biCompression = m_TV->m_DShow->GetFourCC();
				m_TV->bmih_draw.biWidth = m_TV->m_DShow->GetWidth();
				m_TV->bmih_draw.biHeight = m_TV->m_DShow->GetHeight();
				}
			else {
				m_TV->bmih_draw.biBitCount = 24;
				m_TV->bmih_draw.biCompression = BI_RGB;
				m_TV->bmih_draw.biWidth = m_TV->biRawBitmap.biWidth;		//352
				m_TV->bmih_draw.biHeight = m_TV->biRawBitmap.biHeight;
				}
			m_TV->bmih_draw.biSizeImage=(m_TV->bmih_draw.biWidth*m_TV->bmih_draw.biHeight*m_TV->bmih_draw.biBitCount)/8;		// richiesto da MJPG 2019...
			break;
		case WM_SIZE:		// tolto per "deadlock", faccio direttamente sopra quando arriva SPS (RTSP)
			m_TV->bmih_draw.biWidth = m_TV->biRawBitmap.biWidth;		//352
			m_TV->bmih_draw.biHeight = m_TV->biRawBitmap.biHeight;
			break;
		case WM_PAINT:
...
//				return 0;		// PATCH per la merda di Logitech che ignora il setCaptureParms per quanto riguarda i frame per sec. (v. sopra)
			...
			if(lpVHdr.lpData) {
				...
				if(!bInSend) {
					...
					if(theApp.theServer) {
						v=theApp.theServer->getView();
						if(m_TV->qualeInput<2)
							m_TV->compressAFrame(&lpVHdr,m_TV->m_DShow->doPreview,v,theApp.theServer->theHDD,&m_TV->bmih_draw);
						else
							m_TV->compressAFrame(&lpVHdr,TRUE,v,theApp.theServer->theHDD,&m_TV->bmih_draw);
						}
					bInSend=FALSE;
					}
				else {
...
#pragma message (" AUDIO dwBufferLength")
...
#pragma message (" FINIRE canSendAudio")
...
#pragma message (" AUDIO dwBufferLength2")
...
int CTV::compressAFrame(LPVIDEOHDR lpVHdr,bool doPreview,CWnd *v,HDRAWDIB hdd,LPBITMAPINFOHEADER bmih) {
	BYTE *pInp,*pSBuf=NULL,*pSBuf2=NULL;
	BOOL pInpAllocated=FALSE;
  int i,q,tFrame;
	DWORD l,t;
  struct AV_PACKET_HDR *avh;
	...
	if(doPreview) {
		...
		if(qualeInput<2)
			...
			theApp.theServer->getView()->GetClientRect(&rc);
			i=StretchDIBits(hdc->m_hDC,0,0,rc.right/* bmih->biWidth*/,rc.bottom /*bmih->biHeight*/,
				0,0,bmih->biWidth,bmih->biHeight,
				lpVHdr->lpData,(BITMAPINFO*)bmih,DIB_RGB_COLORS,SRCCOPY);
			}
		v->ReleaseDC(hdc);
...
DWORD CTV::RTSPvideo_Thread(LPVOID Param) {
	CVidsendDoc2 *d=(CVidsendDoc2*)Param;
	int i;

	//Used in creating a thread for 

	//Accepts the "this" pointer and casts it to a pointer to .
	if(Param == NULL)
		return -1;

	int retVal=RTSPvideo(d);
	return retVal;
	}

//HANDLE CTV::hHeap=NULL;
DWORD WINAPI CTV::RTSPvideo(CVidsendDoc2 *d) {
  DWORD dwHandleSignaled;
//	HANDLE theEvent=NULL;
	int i,n;
	MSG msg;

	CVidsendDoc2 *parent = (CVidsendDoc2*)d;

  int iRet;
	CDecoderH264 *pDecoder=new CDecoderH264(parent->theTV->m_RTSPsocket,CM_RGB);
	CDecoderH264 *pDecoder2=new CDecoderH264(parent->theTV->m_RTSPsocket2,CM_RGB);
	CDecoderH264 *pDecoder3=new CDecoderH264(parent->theTV->m_RTSPsocket3,CM_RGB);
	CDecoderH264 *pDecoder4=new CDecoderH264(parent->theTV->m_RTSPsocket4,CM_RGB);
  DecodedPicList *pDecPicList,*pDecPicList2,*pDecPicList3,*pDecPicList4;
  InputParameters InputParams;
	uint8_t *allImages=new uint8_t[2000000]; 


	    theApp.FileSpool->print(CLogFile::flagError, "RTSPvideo thread ");


  //get input parameters;
//  Configure(&InputParams, argc, argv);
  memset(&InputParams, 0, sizeof(InputParameters));
//  strcpy(InputParams.infile, /*"test_video_recv2.h264"*/"test_640x360.h264"); //! set default bitstream name
//  strcpy(InputParams.outfile, "test_video_recv.h264"); //! set default output file name
//  strcpy(InputParams.reffile, "test_video_recv.txt"); //! set default reference file name
//	InputParams.poc_scale=1;		// se no dà divide by zero...
//	InputParams.outputFormat=CM_RGB;
//	InputParams.FileFormat=PAR_OF_GDRTP;
//	InputParams.silent=TRUE;
//	CTV::hHeap = HeapCreate(0,HEAP_SIZE,0);

  iRet = pDecoder->OpenDecoder(/*&InputParams*/);
  iRet = pDecoder2->OpenDecoder();
  iRet = pDecoder3->OpenDecoder();
  iRet = pDecoder4->OpenDecoder();
  if(iRet != DEC_OPEN_NOERR)  {
    theApp.FileSpool->print(CLogFile::flagError, "Open decoder failed: 0x%x!", iRet);
//    return -1; //failed;
	  }
	seq_parameter_set_rbsp_t sps;		// mettere dinamico per poi liberare
	if(pDecoder->GetSPS(parent->theTV->m_RTSPsocket,&sps) > 0) {
		parent->theTV->biCompDef.bmiHeader.biWidth=parent->theTV->biRawBitmap.biWidth=(sps.pic_width_in_mbs_minus1+1)*MB_BLOCK_SIZE;
		parent->theTV->biCompDef.bmiHeader.biHeight=parent->theTV->biRawBitmap.biHeight=(sps.pic_height_in_map_units_minus1+1)*MB_BLOCK_SIZE;
		parent->theTV->biCompDef.bmiHeader.biSizeImage=parent->theTV->biRawBitmap.biSizeImage=
			(parent->theTV->biRawBitmap.biWidth*parent->theTV->biRawBitmap.biHeight*parent->theTV->biRawBitmap.biBitCount)/8;
		parent->theTV->biBaseRawBitmap=parent->theTV->biRawBitmap;
		parent->theTV->biRawDef.bmiHeader=parent->theTV->biRawBitmap;
		if(parent->theTV->m_hICCo) {
			ICCompressEnd(parent->theTV->m_hICCo);
			parent->theTV->maxFrameSize=ICCompressGetSize(parent->theTV->m_hICCo,&parent->theTV->biBaseRawBitmap,&parent->theTV->biCompDef);
			i=ICCompressBegin(parent->theTV->m_hICCo,&parent->theTV->biBaseRawBitmap,&parent->theTV->biCompDef);
			}
//		::SetWindowPos(parent->theTV->m_hWnd,HWND_BOTTOM,0,0,parent->theTV->biRawBitmap.biWidth,parent->theTV->biRawBitmap.biHeight,SWP_NOACTIVATE | SWP_NOMOVE);
		// incarta il thread @"£$% 
		parent->theTV->bmih_draw.biWidth = parent->theTV->biRawBitmap.biWidth;		//352
		parent->theTV->bmih_draw.biHeight = parent->theTV->biRawBitmap.biHeight;
		parent->theTV->bmih_draw.biSizeImage=(parent->theTV->bmih_draw.biWidth*parent->theTV->bmih_draw.biHeight*
			parent->theTV->bmih_draw.biBitCount)/8;

	//framePerSecond_hard mettere, parent->theTV->m_RTSPsocket->GetTimeRate("video") /    
		}
	i= setjmp(pDecoder->mark);
  if(i==0)
		;
	else {			// su errore, torneremo qua
		jmp_buf tmp;
		memcpy(&tmp,&pDecoder->mark,sizeof(jmp_buf));
			*theApp.FileSpool << "RESTART H264";
		if(i != -2) {			// eccezione, salto
//  FinitDecoder(&pDecPicList);		// a rischio...
		pDecoder->CloseDecoder();
		}
//	HeapDestroy(CTV::hHeap);
//	CTV::hHeap = HeapCreate(0,HEAP_SIZE,0);
  pDecoder->OpenDecoder(/*&InputParams*/);
		memcpy(&pDecoder->mark,&tmp,sizeof(jmp_buf));
//		reinit_decoder(p_Dec);
		parent->theTV->m_RTSPsocket->Flush();
// questa pianta sempre su ocx...		parent->theTV->m_RTSPsocket->FlushMedia("video");	

		}
//	pDecoder->p_Vid->BitStreamFile=(int32_t)parent->theTV->m_RTSPsocket;

	i=0;

	do {
	/*		dwHandleSignaled=MsgWaitForMultipleObjects(0,NULL, FALSE,1000/parent->theTV->framesPerSec, QS_ALLPOSTMESSAGE);
	// non va bene... i pacchetti rimangono in coda ed arrivano solo dopo
	// ma in effetti cmq c'è la Sleep in RTSP GetMediaPacket ecc quindi è sempre fermo
...
*/
 ...
 // do  {
		try {			// non va manco per finta, riprovare in C++
    iRet = pDecoder->DecodeOneFrame(&pDecPicList);
    iRet = pDecoder2->DecodeOneFrame(&pDecPicList2);
    iRet = pDecoder3->DecodeOneFrame(&pDecPicList3);
    iRet = pDecoder4->DecodeOneFrame(&pDecPicList4);
		}
		catch(CException& e) {
				theApp.FileSpool->print(CLogFile::flagError,"eccezione h264: %d",e );
		}



    if(iRet==DEC_EOS || iRet==DEC_SUCCEED) {
			if(pDecPicList[0].bValid) {
				static uint32_t oldTimeCaptured;

//				if(p_Vid->type != I_SLICE)		// frametype
//					goto skippa;
				...
				if(pDecoder->p_Vid->timestamp>oldTimeCaptured) {		// v. Packet: è uguale per tutti i packet di un frame
//					i=parent->theTV->m_RTSPsocket->GetTimeRate("video");
					oldTimeCaptured=pDecoder->p_Vid->timestamp+parent->theTV->m_RTSPsocket->GetTimeRate("video") /    4 /*framesPerSec*/;

					if(parent->theTV) {
						int i,j;
						uint8_t *p,*p2;

						parent->theTV->biRawBitmap.biWidth=352*2;
						parent->theTV->biRawBitmap.biHeight=288*2;
						SetWindowPos(parent->getView()->m_hWnd,NULL,0,0,352*2,288*2,SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOMOVE);

						parent->theTV->bmih_draw.biWidth=352*2;
						parent->theTV->bmih_draw.biHeight=288*2;

						p=pDecoder3->GetVideoBuffer();
						for(i=0; i<288; i++) {
							p2=allImages+(i*352*3*2);
							for(j=0; j<352*3; j++) {
								*p2++=*p++;
								}
							}
						p=pDecoder4->GetVideoBuffer();
						for(i=0; i<288; i++) {
							p2=allImages+(i*352*3*2)+352*3;
							for(j=0; j<352*3; j++) {
								*p2++=*p++;
								}
							}
						p=pDecoder->GetVideoBuffer();
						for(i=288; i<288*2; i++) {
							p2=allImages+(i*352*3*2);
							for(j=0; j<352*3; j++) {
								*p2++=*p++;
						...
						PostMessage(parent->theTV->m_hWnd,WM_GRABBED_BUFFER,
							(WPARAM)352*2*288*2*3,(LPARAM)allImages);
						}

/*					i++;
				if(i>100) {
		parent->theTV->m_RTSPsocket->Flush();		// fare ogni tanto in caso fossimo in ritardo con i pacchetti...

	provare parent->theTV->FlushMedia();
		i=0;
				}*/

					}

				pDecPicList[0].bValid=0;
i++;

				}
			else
				theApp.FileSpool->print(CLogFile::flagError,"picture non valid: %d",iRet);

// no non serve			delete &pDecPicList[0];

		  }
    else    {
      //error handling;
//      fprintf(stderr, "Error in decoding process: 0x%x\n", iRet);
			theApp.FileSpool->print(CLogFile::flagError,"Error in decoding process: %d",iRet);
//			reinit_decoder(p_Dec);
			}
//		} while((iRet == DEC_SUCCEED) && ((p_Inp->iDecFrmNum==0) || (iFramesDecoded<p_Inp->iDecFrmNum)));

#if 0
	 if(HeapLock(CTV::hHeap)) {		// https://learn.microsoft.com/en-us/windows/win32/memory/enumerating-a-heap
     PROCESS_HEAP_ENTRY Entry;

		int nbl=0;
		Entry.lpData = NULL;
    while(HeapWalk(CTV::hHeap, &Entry)) {
      if(Entry.wFlags & PROCESS_HEAP_ENTRY_BUSY) {
//					theApp.FileSpool->print(CLogFile::flagError,"Busy block in heap: %08X, Size: %d bytes",
  //               Entry.lpData, Entry.cbData);
			  nbl++;
			}
			}
					theApp.FileSpool->print(CLogFile::flagError,"memory blocks %u, iret=%d, valid=%d",nbl, iRet,pDecPicList[0].bValid);
extern CMap <void *,void*,uint16_t,uint16_t&> MEMORY_LIST;
//					theApp.FileSpool->print(CLogFile::flagError,"  MEMORY LIST %u",MEMORY_LIST.GetCount());
    if(HeapUnlock(CTV::hHeap) == FALSE)
			;
...
if(0  /*i>1000*/) {
			...
			*theApp.FileSpool << "reset mem------";
		jmp_buf tmp;
		memcpy(&tmp,&pDecoder->mark,sizeof(jmp_buf));

  pDecoder->FinitDecoder(&pDecPicList);		// a rischio...
  pDecoder->CloseDecoder();
...
/*	 if(HeapLock(CTV::hHeap)) {		// https://learn.microsoft.com/en-us/windows/win32/memory/enumerating-a-heap
     PROCESS_HEAP_ENTRY Entry;
		...
		Entry.lpData = NULL;
    if(HeapWalk(CTV::hHeap, &Entry))
					theApp.FileSpool->print(CLogFile::flagError,"memory leak");
    if(HeapUnlock(CTV::hHeap) == FALSE)
			;
...
//        _tprintf(TEXT("  Data portion begins at: %#p\n  Size: %d bytes\n") \
//                 TEXT("  Overhead: %d bytes\n  Region index: %d\n\n"),
//				theApp.FileSpool->print(CLogFile::flagError,"Lost Region at %08X, Size: %d bytes",
//                 Entry.lpData, Entry.cbData,
//                 Entry.cbOverhead, Entry.iRegionIndex);
					}
...
//        _tprintf(TEXT("  Data portion begins at: %#p\n  Size: %d bytes\n") \
//                 TEXT("  Overhead: %d bytes\n  Region index: %d\n\n"),
//				theApp.FileSpool->print(CLogFile::flagError,"Uncommitted block at %08X, Size: %d bytes",
//                 Entry.lpData, Entry.cbData,
//                 Entry.cbOverhead, Entry.iRegionIndex);
					}
        else {
...
extern CMap <void *,void*,uint16_t,uint16_t&> MEMORY_LIST;
	  ...
	  typedef DWORD ((WINAPI *FARPROCPARM2)(HANDLE,void*,DWORD));
		...
		if(myInst=LoadLibrary("psapi.dll")) {
			if(myProc2=(FARPROCPARM2)GetProcAddress(GetModuleHandle(TEXT("psapi.dll"))/*myInst*/,"GetProcessMemoryInfo")) {
				(*myProc2)/*GetProcessMemoryInfo*/(GetCurrentProcess(),&pmc,sizeof(pmc));
				}
			FreeLibrary(myInst);
			}

		theApp.FileSpool->print(CLogFile::flagInfo,"memoria: %u,%u",
			pmc.PeakWorkingSetSize/1024,pmc.WorkingSetSize/1024);


	POSITION pos = MEMORY_LIST.GetStartPosition();
	void *nKey;
	uint16_t pt;
	while(pos) {
		MEMORY_LIST.GetNextAssoc(pos, nKey, pt);
		theApp.FileSpool->print(CLogFile::flagError,"lost block at line %d: %08X",pt,nKey);
//		HeapFree(GetProcessHeap(),0,nKey);
		}

	MEMORY_LIST.RemoveAll();
#endif
	...
	i=0;
		...
		}	while(1 /*i<500*/ && !parent->theTV->endThreadV);

endThread:

			*theApp.FileSpool << "thread h264 terminato";

//			ASSERT(0);

  iRet = pDecoder->FinitDecoder(&pDecPicList);
//	HeapFree(GetProcessHeap(),0,InputParams.outbuf);		//delete []InputParams.outbuf;
  iRet = pDecoder->CloseDecoder();
	delete pDecoder;
	delete pDecoder2;
	delete pDecoder3;
	delete pDecoder4;
		parent->theTV->m_RTSPsocket->FlushMedia("video");	//OCCHIO v. sopra!
//	HeapDestroy(CTV::hHeap);

	delete allImages;

	parent->theTV->endThreadV=FALSE;
